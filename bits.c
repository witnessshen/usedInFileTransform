...
int allOddBits(int x) {
  return !~((85|x)|(((85)<<8)|x)|((85<<16)|x)|((85<<24)|x));
}
/* 
 * negate - return -x 
 *   Example: negate(1) = -1.
...
int isLessOrEqual(int x, int y) {
    int diffSign = (x >> 31 & 1) ^ (y >> 31 & 1);        
    int yMinusXSign = ((y + (~x + 1)) >> 31) & 1; 
    return (diffSign & (x >> 31 & 1)) | (!diffSign & !yMinusXSign);
}
//4
/* 
...
unsigned floatScale2(unsigned uf) {
    ...
    unsigned exponent = (uf >> 23) & 0xFF; // 提取指数部分
    unsigned mantissa = uf & 0x7FFFFF;     // 提取尾数部分
    ...
    if (exponent == 0xFF) {
        ...
        // 正规化数：增加指数值
        exponent += 1;
        // 如果指数增加后变为 255，表示为无穷大
        if (exponent == 0xFF) {
            mantissa = 0;  // 将尾数设置为 0 表示无穷大
        }
